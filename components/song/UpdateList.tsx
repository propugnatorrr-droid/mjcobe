...
export async function UpdateList({ updates }: { updates: SongPageData['updates'] }) {
  return (
    <section className="py-16 md:py-24">
      <Eyebrow>{await text('song.section.updates')}</Eyebrow>

      {updates.length === 0 ? (
        <p className="mt-8 max-w-[62ch] text-body text-[var(--text-dim)]">
          {await text('song.empty.updates')}
        </p>
      ) : (
        <div className="mt-10 border-t border-[var(--line)]">
          {await Promise.all(
            updates.map(async (update) => {
              const locked = update.minTierCents > 0;
              return (
                <article
                  key={update.id}
                ...
                >
                  <span className="font-mono text-eyebrow uppercase text-[var(--text-dim)]">
                    {update.publishedAt ? await formatDay(update.publishedAt) : ''}
                  </span>
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-4">
                      ...
                      {locked ? (
                        <Tag>{await text('song.updates.locked_label')}</Tag>
                      ) : null}
                    </div>

                    <p className="mt-4 max-w-[62ch] text-body text-[var(--text-dim)]">
                      {locked
                        ? await text('song.updates.locked', {
                            amount: formatCents(cents(update.minTierCents)),
                          })
                        : update.body}
                    </p>
                  </div>
